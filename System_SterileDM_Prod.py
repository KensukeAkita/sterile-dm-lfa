...
from DOF import grho_below120MeV, dgrhodT_below120MeV, grho_above120MeV, dgrhodT_above120MeV
from Asy_Redistribution import eqs_redist
from Thermo_quantities import Thermo_quantities

def System_SterileDM_Prod(T,sys_values,Gamma_a_Table,Gamma_a_anti_Table,s_ini,ms,Uas):
    ...
    rho, s, P, Delta_rho, rho_nua, rho_a, Del_nua, Del_a, Del_nu_total, Del_c_total, Del_Q_QCD \
          = Thermo_quantities(T,zeta_nue,zeta_numu,zeta_nutau,zeta_B,zeta_Q)
    ...
    ans2 =  sp.optimize.root(eqs_redist, x0, args = (T2,La), method='hybr',tol=tol_root)  
...
def Kinetic_eq(T,sys_values,Gamma_a_Table,Gamma_a_anti_Table,zeta_nua,s_ini,rho,s,P,dDelta_rhodT,rho_nua, rho_a, Del_nua, Del_a, Del_nu_total, Del_c_total, Del_Q_QCD,ms,Uas):
    ...
    for ni in range(n): #bin for neutrino spectrum 
        ...
        y_scale = y2[ni] #y_scale = (s_ini/s)^(1/3)p/T
        dy_scale = dy2[ni]
